    ...
    try:
        ...
        for ap_mac in ap_macs:
            ...
            ap_device = device_registry.async_get_or_create(
                ...
                via_device=controller_device.id
            )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    if unload_ok and DOMAIN in hass.data and entry.entry_id in hass.data[DOMAIN]:
        ...
        # Clean up devices
        device_registry = dr.async_get(hass)
        if "ap_device_ids" in entry_data:
            for ap_id in entry_data["ap_device_ids"]:
                device_registry.async_remove_device(ap_id)
